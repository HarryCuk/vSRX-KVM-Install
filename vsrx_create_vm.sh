...
cp junos-vsrx3-x86-64-22.3R1.11.qcow2 /mnt/vsrx20one.qcow2
sleep 2
...
virt-install --name vSrxTest \
 --memory 4096 \
 --cpu host \
 --vcpus=2 \
 --arch=x86_64 \
 --disk /mnt/vsrx20one.qcow2,size=16,device=disk,bus=ide,format=qcow2 --import \
  --os-type linux \
  --os-variant rhel7.0 \
  --network=network:default,model=virtio \
  --network=network:trusted,model=virtio \
  --network=network:untrusted,model=virtio
